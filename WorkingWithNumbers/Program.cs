...
int number2 = 2;
//var number3 = number1 * number2; // Even when using a bigger datatype to store the results we get the overflow because the calculation is done first (between 2 integers) and then stored.
//WriteLine($"number3: {number3:N0}");
WriteLine($"number1: {number1:N0}");
WriteLine($"number2: {number2:N0}");
WriteLine($"number1 * number2: {number1 * number2:N0}");
WriteLine($"Math.BigMul(number1, number2): {Math.BigMul(number1, number2):N0}"); // This method explicitly performs the multiplication in the next bigger (long) space.
...
// New random methods in .NET 8 an later.
