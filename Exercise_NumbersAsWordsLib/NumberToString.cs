...
namespace Packt.Shared
{
    public static class NumberToString
    {
        ...
        // Single-digit and small number names up to 19
        ...
        public static string PacktToWords(this int input)
        {
            return PacktToWords((BigInteger)input);
        }

        public static string PacktToWords(this long input)
        {
            return PacktToWords((BigInteger)input);
        }
        ...
        public static string ToWords(this BigInteger input)
        {
            ...
            else
            {
                if (input < 20)
                {
                    res = smallNumbers[(int)input];
                }
                else if (input < 100)
                {
                    int teners = (int)input / 10;
                    int singles = (int)input - (teners * 10);
                    res = tens[teners] + smallNumbers[singles];
                }
                else if (input < 1000)
                {
                    int hundreds = (int)input / 100;
                    int teners = (int)input %100 / 10;
                    int singles = (int)input - (hundreds * 100) - (teners * 10);
                    res = smallNumbers[hundreds] + "hundred " + tens[teners] + smallNumbers[singles];
                }
                else if (input < 10000)
                {
                    int thousands = (int)input / 1000;
                    int hundreds = (int)input % 1000 / 100;
                    int teners = (int)input % 100 / 10;
                    int singles = (int)input - (thousands * 1000) - (hundreds * 100) - (teners * 10);
                    res = smallNumbers[thousands] + "thousand " + smallNumbers[hundreds] + "hundred " + tens[teners] + smallNumbers[singles];
                }
            }
            return res;
        }
        ...
        public static string PacktToWords(this BigInteger number)
        {
            /* 
            ...
            */
            ...
            string ThreeDigitGroupToWords(int threeDigits)
            {
                ...
                */
                ...
                if (tens >= 2)
                {
                    groupText += NumberToString.tens[tens];
                    if (units != 0)
                    {
